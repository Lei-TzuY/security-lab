...
- after a successful authenticated response publication, the wrapper records exactly one outstanding tuple of request ID plus SHA-256 of the complete response bytes. No second response may be published and no later request packet may be consumed until that acknowledgment barrier clears;
...
- broker-level integration uses the real readiness handshake and `SCM_RIGHTS` endpoint grant, leaves a newly queued request untouched while response 42 awaits acknowledgment, clears that barrier only after the exact response digest is authenticated, then completes later response/acknowledgment pairs;
