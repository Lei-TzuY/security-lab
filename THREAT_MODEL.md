...
- The policy author is trusted to choose filesystem exposure, including any declared read-only host-volume source/target, stdio exposure, selected already-open object handles, target data/syscall grants, resource ceilings, capture ceilings, and any wall-clock deadline. Selecting a handle intentionally grants the authority already represented by that open file description.
