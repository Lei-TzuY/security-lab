...
# Policy surface: one source FD plus one target-visible broker channel FD.
replace_one(
    "src/policy.rs",
    ...
    "    pub host_unix_stream_peer_uid: Option<u32>,\n    pub host_unix_stream_peer_gid: Option<u32>,\n    /// Optional one-shot runtime descriptor broker. The source names an already-open\n    /// caller FD pinned before fork; the target names the AF_UNIX control channel\n    /// installed into the direct target. The descriptor itself is transferred only\n    /// after the exec'd target sends the broker readiness byte.\n    pub runtime_fd_broker_source_fd: Option<u32>,\n    pub runtime_fd_broker_target_fd: Option<u32>,\n    /// Optional launcher-brokered TCP listener bound only to host 127.0.0.1.\n",
...
replace_one(
    ...
    "        assert_eq!(policy.host_unix_stream_peer_uid, None);\n        assert_eq!(policy.host_unix_stream_peer_gid, None);\n        assert_eq!(policy.runtime_fd_broker_source_fd, None);\n        assert_eq!(policy.runtime_fd_broker_target_fd, None);\n        assert!(policy.landlock_read_execute.is_empty());\n",
...
# Runtime preparation and host-side one-shot SCM_RIGHTS service.
replace_one(
    "src/platform/linux.rs",
    "    const PHASE_PROCFS_MOUNT: u32 = 57;\n    const PHASE_PROCFS_PID1_HARDEN: u32 = 58;\n",
    "    const PHASE_PROCFS_MOUNT: u32 = 57;\n    const PHASE_PROCFS_PID1_HARDEN: u32 = 58;\n    const PHASE_RUNTIME_FD_BROKER_ISOLATE: u32 = 59;\n",
...
replace_one(
    ...
    "storage floor includes broker channel",
)
...
replace_one(
    ...
    "selected handle broker capacity",
)
...
replace_one(
    ...
    "mutable prepared for broker take",
)
...
replace_one(
    ...
    "        let runtime_fd_broker = prepared.runtime_fd_broker.take();\n        let runtime_fd_broker_thread = runtime_fd_broker.map(|broker| {\n            std::thread::spawn(move || service_runtime_fd_broker(broker))\n        });\n        // The host parent does not retain launcher-owned duplicates of selected\n        // object capabilities while the target runs. Caller-owned source FDs\n        // remain under caller control. The one runtime-broker source duplicate\n        // is owned only by the post-fork host broker thread until transfer/EOF.\n        drop(prepared);\n",
    "spawn broker thread",
)
...
replace_one(
    ...
    "child closes host broker state",
)
...
# Test helper and deterministic integration evidence.
replace_one(
    "tests/sandbox.rs",
...
replace_one(
    ...
    "runtime broker raw data",
)

# Static authority observability must expose this explicit post-launch capability.
replace_one(
