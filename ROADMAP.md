...
- `executable.needed` / `executable.needed_sha256` are an all-or-nothing restriction that requires both `executable.sha256` and the sealed `PT_INTERP` binding; the dependency path is absolute/literal, rejects dynamic-linker `# Roadmap
...
Boundary: 49A binds only the initial ELF64 x86_64 `PT_INTERP` loader selected by the already content-bound main image. The expected digest is not signer/provenance evidence. This slice does not bind shared libraries opened by the loader, statically linked images without `PT_INTERP`, later target `execve` or explicitly granted later `execveat`, or provide a process-lifetime executable closure.
 ...
 tokens, differs from executable/interpreter, and cannot overlap private procfs, scratch, or persistent-volume targets;
- a bounded ELF64 little-endian x86_64 parser walks `PT_DYNAMIC`, resolves `DT_STRTAB` through one file-backed `PT_LOAD`, bounds dynamic/string-table work, rejects malformed duplicate/unterminated metadata, and extracts direct `DT_NEEDED` strings without executing the target;
- the sealed main executable must contain exactly one direct `DT_NEEDED` entry byte-for-byte equal to the configured path; the launcher then pins that regular executable shared object beneath the root, identity-revalidates it while hashing/copying, requires the exact SHA-256 under the existing 64 MiB ceiling, and retains an immutable sealed memfd image;
- interpreter and direct dependency now reuse one generic sealed single-file mount path: a private tmpfs copy is cloned, hardened read-only + `nosuid,nodev`, and attached at the exact final-root pathname before chroot/exec;
- configured-filesystem preflight read-only verifies the sealed main ELF's exact direct-needed entry and dependency digest; static authority manifest/delta model the path+digest pair as one optional exact restriction;
- deterministic integration builds a real shared object with SONAME `/dependency` and a PIE that directly calls its exported symbol. The content-bound PIE embeds path-qualified `DT_NEEDED=/dependency`; under sealed main+interpreter+dependency bindings it executes through the real dynamic loader and exits 91. Wrong dependency digest or a configured path absent from the sealed main's direct `DT_NEEDED` list fails before target execution, and the trusted parent proves the host dependency bytes are unchanged;
- exact candidate rustfmt, Clippy with `-D warnings`, complete stable tests, and the complete Rust 1.74 suite are green.
