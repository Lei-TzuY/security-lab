use security_lab::{ChildOutcome, CowDiff, CowDiffEntry, RunReport, SandboxError};
...
pub(crate) fn report_json(report: &RunReport) -> String {
    ...
    for (index, volume) in report.cow_volume_diffs.iter().enumerate() {
        ...
        push_hex(&mut output, volume.target.as_os_str().as_bytes());
...
mod tests {
    ...
    #[test]
    fn serializes_volume_cow_diff_with_target_identity() {
        let report = RunReport {
            outcome: ChildOutcome::Exited(0),
            stdout: None,
            cow_diff: None,
            cow_volume_diffs: vec![security_lab::CopyOnWriteVolumeDiff {
                target: std::path::PathBuf::from("/state"),
                diff: CowDiff {
                    entries: vec![CowDiffEntry::UpsertFile {
                        path: b"/item".to_vec(),
                        mode: 0o600,
                        bytes: b"ok".to_vec(),
                    }],
        ...
        assert!(json.contains(
            "\"cow_volume_diffs\":[{\"target_encoding\":\"hex\",\"target\":\"2f7374617465\",\"diff\":{\"encoded_bytes\":32"
        ));
        assert!(json.contains("\"path\":\"2f6974656d\""));
    }
